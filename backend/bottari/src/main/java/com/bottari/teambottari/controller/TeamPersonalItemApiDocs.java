...
@Tag(name = "Team Personal Item", description = "팀 보따리 개인 아이템 API")
public interface TeamPersonalItemApiDocs {

    @Operation(summary = "개인 물품 체크")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "개인 물품 체크 성공"),
    })
    @ApiErrorCodes({
    ...
    @Operation(summary = "개인 물품 체크 해제")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "개인 물품 체크 해제 성공"),
    })
    @ApiErrorCodes({
            ErrorCode.TEAM_BOTTARI_ITEM_NOT_FOUND,
