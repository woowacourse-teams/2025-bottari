...
@WebMvcTest(TeamAssignedItemController.class)
@Import({LogFormatter.class, WebConfig.class})
class TeamAssignedItemControllerTest {
    ...
    void check() throws Exception {
        ...
        willDoNothing().given(teamAssignedItemService)
                .check(itemId, ssaid);

        // when & then
        mockMvc.perform(patch("/team-assigned-items/{id}/check", itemId)
                        .header("ssaid", ssaid))
                .andExpect(status().isNoContent());
    }

    @DisplayName("팀 담당 아이템을 체크 해제한다.")
    @Test
    void uncheck() throws Exception {
        ...
        // when & then
        mockMvc.perform(patch("/team-assigned-items/{id}/uncheck", itemId)
                        .header("ssaid", ssaid))
                .andExpect(status().isNoContent());
    }
}
